...
const CURRENCY_SYMBOLS = [
  ...
  { sym:"JPY",color:"#f87171" }, { sym:"BTC",color:"#fb923c" },
  { sym:"ETH",color:"#34d399" }, { sym:"GOLD",color:"#fbbf24" },
  { sym:"%",  color:"#a78bfa" }, { sym:"₿",  color:"#fb923c" },
];
    ...
    <div style={c.page}>
      <style>{`
        ...
        *{box-sizing:border-box;}body{margin:0;font-family:'Inter',sans-serif;}
        @keyframes floatCoin{0%{transform:translateY(0) rotate(0deg) scale(1);opacity:.18}50%{transform:translateY(-32px) rotate(8deg) scale(1.08);opacity:.32}100%{transform:translateY(0) rotate(0deg) scale(1);opacity:.18}}
      ...
      <div style={{position:"fixed",inset:0,pointerEvents:"none",overflow:"hidden",zIndex:0}} aria-hidden="true">
        {FLOAT_POS.map((f,i)=>(
          <span key={i} style={{
            ...
            userSelect:"none", pointerEvents:"none", letterSpacing:"-0.5px",
      ...
      <div style={{...c.card, animation:"fadeUp 0.5s ease both"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:28}}>
          <div style={sharedStyles.logoBox}>
        ...
        <h1 style={c.title}>Welcome back</h1>
        <p style={c.subtitle}>Sign in to your account to continue</p>
        ...
        {error && <div style={sharedStyles.errorBox}>{error}</div>}
        ...
        <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:16}}>
          <div>
            ...
            <input id="login-email" name="email" type="email" required value={form.email}
              ...
              className="login-input" style={c.input}/>
          </div>
          <div>
            <label style={c.label}>Password</label>
            <input id="login-password" name="password" type="password" required value={form.password}
              onChange={handleChange} placeholder="Enter your password"
              className="login-input" style={c.input}/>
          </div>

          <button id="login-submit" type="submit" disabled={loading} className="login-btn"
            style={{...sharedStyles.submitBtn,opacity:loading?0.7:1,transition:"opacity 0.2s,transform 0.2s"}}>
        ...
        <p style={c.footerTxt}>
          ...
          <Link to="/register" style={sharedStyles.link}>Create one</Link>
        </p>
      </div>
    </div>
  );
...
const sharedStyles = {
  ...
  successBox: { background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:10,padding:"10px 14px",color:"#15803d",fontSize:13,marginBottom:18,fontWeight:500 },
  errorBox:   { background:"#fef2f2",border:"1px solid #fecaca",borderRadius:10,padding:"10px 14px",color:"#dc2626",fontSize:13,marginBottom:18,fontWeight:500 },
};
...
const D = {
  ...
  card:       { position:"relative",zIndex:10,width:"100%",maxWidth:420,background:"#1e293b",border:"1px solid rgba(255,255,255,0.1)",borderRadius:24,padding:"40px 36px",backdropFilter:"blur(24px)",boxShadow:"0 24px 64px rgba(0,0,0,0.5)" },
  logoName:   { fontWeight:800,fontSize:16,color:"#fff",letterSpacing:"-0.3px" },
  logoSub:    { fontSize:11,color:"#64748b",fontWeight:500 },
  title:      { fontSize:26,fontWeight:800,color:"#f1f5f9",margin:"0 0 6px",letterSpacing:"-0.5px" },
  ...
  input:      { width:"100%",padding:"11px 14px",borderRadius:10,border:"1px solid rgba(255,255,255,0.12)",background:"rgba(255,255,255,0.08)",color:"#f1f5f9",fontSize:14,transition:"border-color 0.2s",boxSizing:"border-box" },
  footerTxt:  { textAlign:"center",marginTop:24,fontSize:13,color:"#64748b" },
};
...
const L = {
  ...
  card:       { position:"relative",zIndex:10,width:"100%",maxWidth:420,background:"rgba(255,255,255,0.92)",border:"1px solid rgba(99,102,241,0.15)",borderRadius:24,padding:"40px 36px",backdropFilter:"blur(24px)",boxShadow:"0 20px 60px rgba(99,102,241,0.12),0 4px 16px rgba(0,0,0,0.06)" },
  logoName:   { fontWeight:800,fontSize:16,color:"#1e1b4b",letterSpacing:"-0.3px" },
  logoSub:    { fontSize:11,color:"#94a3b8",fontWeight:500 },
  title:      { fontSize:26,fontWeight:800,color:"#1e1b4b",margin:"0 0 6px",letterSpacing:"-0.5px" },
  ...
  footerTxt:  { textAlign:"center",marginTop:24,fontSize:13,color:"#94a3b8" },
};
