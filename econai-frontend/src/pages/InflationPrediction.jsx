...
const NUMERIC_FIELDS = [
  ...
  { name: "inflation",             label: "Previous Inflation",  placeholder: "e.g. 3.5", unit: "%" },
...
export default function InflationPrediction() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setResult(Number(res?.data?.predicted_inflation ?? 0));
      setShowInsights(true);
    } catch { setError("Failed to connect to the ML service. Is the server running?"); }
    finally { setLoading(false); }
  };

  const inflState = result === null ? null
    : result <= 2.5 ? { label: "Within Target", color: "#22c55e", bg: "#f0fdf4", border: "#bbf7d0", darkBg: "rgba(34,197,94,0.15)"  }
    : result <= 5.0 ? { label: "Elevated",       color: "#f59e0b", bg: "#fffbeb", border: "#fde68a", darkBg: "rgba(245,158,11,0.15)" }
    :                 { label: "High Inflation",  color: "#ef4444", bg: "#fef2f2", border: "#fecaca", darkBg: "rgba(239,68,68,0.15)"  };

  const resultPanel = (
    <div style={{
      position: "sticky", top: 24, borderRadius: 12, padding: 24,
      border: inflState ? `1px solid ${inflState.border}` : t.cardBorder,
      background: inflState
        ? (t.text === "#e2e8f0" ? inflState.darkBg : inflState.bg)
        : t.cardBg,
      ...
      {!result && !loading ? (
        <div style={{ textAlign: "center", color: t.textSubtle }}>
          ...
          <div style={{ fontSize: 12, marginTop: 4, color: t.textSubtle }}>Run forecast to see inflation</div>
        </div>
      ) : loading ? (
        ...
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: inflState.color, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>Predicted Inflation</div>
          <div style={{ fontSize: 58, fontWeight: 800, color: inflState.color, lineHeight: 1, letterSpacing: "-2px", marginBottom: 12 }}>
            {result.toFixed(2)}<span style={{ fontSize: 24, fontWeight: 700 }}>%</span>
          </div>
          <div style={{ display: "inline-block", background: inflState.color, color: "#fff", fontSize: 12, fontWeight: 700, padding: "4px 10px", borderRadius: 6, textTransform: "uppercase", letterSpacing: "0.05em" }}>
            {inflState.label}
          </div>
        </div>
      )}
  ...
  return (
    <PredictionShell
      accent={ACCENT}
      title="Inflation Forecast Screener"
      subtitle="Predict consumer price inflation using GradientBoosting Regressor (R²: 0.99)"
      ...
      insightsSection={showInsights && <>
        <AIExplanation
          context="Inflation Forecast Analysis"
          predictionData={{ indicators: form, predictedInflation: `${result?.toFixed(2)}%`, outlook: inflState.label }}
          newsQuery="inflation consumer prices CPI central bank"
          accentColor={ACCENT}
        />
        <PredictionNews query="inflation consumer prices CPI central bank" accentColor={ACCENT} />
      </>}
    />
  );
}
